...
TXT_OUTPUT_PATH = "/Users/nicolasbancel/Desktop/MindedTech_Suger/LISA/revised_actions"  # Need to create the folder and paste the path

# Shaden's path
# /Users/shaden/Documents/LISA/revised_actions
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "You are a translation assistant revising an English"
                    " version based on expert‑edited French input."
                ),
            },
...
def process_sheet():
    ...
    col_map = {
        name: idx + 1 for idx, name in enumerate(headers)
    }  # added for col separation
    ...
    for i, row in enumerate(data):
        ...
        try:
            ...
            filename_title = (
                row.get("EN_Title", f"row_{row_number}")
                .strip()
                .replace(" ", "_")
                .replace("/", "-")
            )
